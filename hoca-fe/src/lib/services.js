...
export const authApi = {
  ...
  revokeSession: (sessionId) => api.delete(`/api/auth/sessions/${sessionId}`).then((r) => r.data),
  logout: () => api.post('/api/auth/logout').then((r) => r.data),
  logoutAll: () => api.post('/api/auth/logout-all').then((r) => r.data),
  begin2fa: () => api.post('/api/auth/2fa/setup').then((r) => r.data),
  confirm2fa: (code) => api.post('/api/auth/2fa/confirm', { code }).then((r) => r.data),
  disable2fa: (password, code) => api.post('/api/auth/2fa/disable', { password, code }).then((r) => r.data),
};
...
export const userApi = {
  ...
  deleteMe: (password, code) =>
    api.delete("/api/users/me", { data: { password, code } }).then((r) => r.data),
  requestEmailChange: (password, newEmail) => api.post('/api/users/me/email-change-request', { password, newEmail }).then((r) => r.data),
  confirmEmailChange: (code) => api.post('/api/users/me/email-change-confirm', { code }).then((r) => r.data),
  ...
  exportData: () => api.get('/api/users/me/export', { responseType: 'blob' }).then((r) => r.data),
  recoverStreak: () =>
...
export const roomApi = {
  ...
  respondInvitation: (id, status) => api.patch(`/api/rooms/invitations/${id}`, { status }).then((r) => r.data),
  invite: (roomId, userId) => api.post(`/api/rooms/${roomId}/invites`, { userId }).then((r) => r.data),
  inviteEmail: (roomId, email) => api.post(`/api/rooms/${roomId}/invites`, { email }).then((r) => r.data),
  ...
  unfavorite: (id) => api.delete(`/api/rooms/${id}/favorite`).then((r) => r.data),
  export: (id) => api.get(`/api/rooms/${id}/export`, { responseType: 'blob' }).then((r) => r.data),
  rate: (id, rating, comment) => api.post(`/api/rooms/${id}/rating`, { rating, comment }).then((r) => r.data),
};
...
export const notificationApi = {
  ...
  markRead: (notificationIds) =>
    api
      ...
      .then((r) => r.data),
  archive: (id) => api.patch(`/api/notifications/${id}/archive`).then((r) => r.data),
...
export const aiApi = {
  ...
  ask: (question, history = [], options = {}) =>
    api.post("/api/ai/ask", { question, history, ...options }).then((r) => r.data),
  listConversations: () => api.get("/api/ai/conversations").then((r) => r.data),
  getConversation: (id) => api.get(`/api/ai/conversations/${id}`).then((r) => r.data),
  updateConversation: (id, data) => api.patch(`/api/ai/conversations/${id}`, data).then((r) => r.data),
  deleteConversation: (id) => api.delete(`/api/ai/conversations/${id}`).then((r) => r.data),
  rateMessage: (id, messageId, feedback) => api.patch(`/api/ai/conversations/${id}/messages/${messageId}/feedback`, { feedback }).then((r) => r.data),
};
...
export const studyGoalApi = {
  ...
  create: (goal) => api.post("/api/study-goals", typeof goal === "string" ? { text: goal } : goal).then((r) => r.data),
  complete: (id) =>
    api.patch(`/api/study-goals/${id}/complete`).then((r) => r.data),
  update: (id, data) => api.patch(`/api/study-goals/${id}`, data).then((r) => r.data),
...
export const publicApi = {
  ...
  getSystemHealth: () => api.get("/health").then((r) => r.data).catch((error) => {
...
export const paymentApi = {
  ...
  downloadReceipt: (transactionId) =>
    api.get(`/api/payment/transactions/${transactionId}/receipt`, { responseType: "blob" }).then((r) => r.data),
  retryTransaction: (transactionId) =>
    api.post(`/api/payment/transactions/${transactionId}/retry`).then((r) => r.data),
  requestRefund: (transactionId, reason) =>
    api.post(`/api/payment/transactions/${transactionId}/refund`, { reason }).then((r) => r.data),
  ...
  publicPayosStatus: (orderCode) =>
    api.get(`/api/payment/payos/public-status/${orderCode}`).then((r) => r.data),
...
export const supportApi = {
  ...
  mine: (page = 1) => api.get('/api/support/mine', { params: { page } }).then((r) => r.data),
  get: (id) => api.get(`/api/support/${id}`).then((r) => r.data),
  reply: (id, message, attachments = []) => api.post(`/api/support/${id}/reply`, { message, attachments }).then((r) => r.data),
  adminList: (status) => api.get('/api/support/admin', { params: status ? { status } : {} }).then((r) => r.data),
  setStatus: (id, status) => api.patch(`/api/support/${id}/status`, { status }).then((r) => r.data),
};

// ============ SMART DISCUSSION ============
export const discussionApi = {
  get: (roomId) =>
    api.get(`/api/discussions/${roomId}`).then((r) => r.data),
...
export const uploadApi = {
  supportAttachment: (file) => {
    ...
    return api.post("/api/upload/support-attachment", fd, { headers: { "Content-Type": "multipart/form-data" } }).then((r) => r.data);
  },
  avatar: (file) => {
...
export const adminApi = {
  ...
  warnUser: (id, data) => api.post(`/api/admin/users/${id}/warn`, data).then((r) => r.data),
  blockUser: (id) =>
  ...
  getTransactions: (params = {}) => api.get('/api/admin/revenue/transactions', { params }).then((r) => r.data),
  getAnalytics: (params = {}) => api.get('/api/admin/analytics', { params }).then((r) => r.data),
  getSystemConfig: () => api.get('/api/admin/config').then((r) => r.data),
  updateSystemConfig: (data) => api.put('/api/admin/config', data).then((r) => r.data),
  getNotifications: (params = {}) => api.get('/api/admin/notifications', { params }).then((r) => r.data),
  markNotificationsRead: (notificationIds) => api.post('/api/admin/notifications/mark-read', { notificationIds }).then((r) => r.data),
  archiveNotification: (id) => api.patch(`/api/admin/notifications/${id}/archive`).then((r) => r.data),
  deleteNotification: (id) => api.delete(`/api/admin/notifications/${id}`).then((r) => r.data),
  getAdViews: (params = {}) => api.get('/api/admin/ads/views', { params }).then((r) => r.data),
  ...
  toggleAdPlacementEnabled: (id) =>
    api.post(`/api/admin/ads/placements/${id}/toggle-enabled`).then((r) => r.data),
};
