...
const signToken = (id, role, subscriptionTier) => {
  return jwt.sign({ id, role, subscriptionTier }, JWT_SECRET, {
    expiresIn: "7d",
  });
};
...
const registerUser = async (userData) => {
  ...
  // Create user with ACTIVE status (skip email verification for development)
  const user = await User.create({
    displayName,
    email,
    password,
    accountStatus: "ACTIVE", // Changed from INACTIVE to ACTIVE
    verificationCode,
  ...
  // Return user info with token (email verification skipped for development)
